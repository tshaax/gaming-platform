...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-gradient-to-br from-amber-900 via-orange-800 to-orange-900 flex items-center justify-center px-4">
      <div class="w-full max-w-md">
        <div class="bg-white/10 backdrop-blur-md rounded-2xl shadow-2xl p-8 border border-white/20">
          <div class="text-center mb-8">
            ...
            <h1 class="text-4xl font-bold text-white mb-2">Cashier</h1>
            <p class="text-amber-200">Login to your cashier account</p>
          </div>

          <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-6">
            <!-- Credential Type Toggle -->
            <div>
              <p class="block text-sm font-medium text-amber-100 mb-3">Login With</p>
              <div class="flex gap-2">
                <button
                  type="button"
                  (click)="toggleCredentialType()"
                  [class.bg-amber-600]="credentialType() === 'email'"
                ...
                <button
                  type="button"
                  (click)="toggleCredentialType()"
                  [class.bg-amber-600]="credentialType() === 'cellphone'"
            ...
            <div>
              <label [for]="'credential-input'" class="block text-sm font-medium text-amber-100 mb-2">
              ...
              <input
                ...
                [placeholder]="credentialType() === 'email' ? 'cashier@example.com' : '+1 (555) 000-0000'"
                class="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-amber-300 focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-400 focus:ring-opacity-50"
              />
            </div>

            <!-- Password Input -->
            <div>
              <label for="password" class="block text-sm font-medium text-amber-100 mb-2">Password</label>
              <input
                id="password"
                type="password"
            ...
            <button
              ...
              class="w-full py-3 px-4 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white font-semibold rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              @if (isLoading()) {
